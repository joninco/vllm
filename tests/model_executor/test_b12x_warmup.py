    ...
    def __post_init__(self):
        if self.plan is None:
            self.plan = SimpleNamespace(
                name=self.name, prepared=None, scratch_specs=lambda: ()
            )
...
def test_collect_units_filters_by_stage_and_forces_eager_defaults() -> None:
    record: list[tuple[torch.nn.Module, B12xWorkload]] = []
...
def test_profile_prepares_collectives_and_releases_only_fresh_plans(
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(reserve_all=lambda: None),
    )
    ...
    installed = _Request(
        ...
        plan=SimpleNamespace(
            name="installed", prepared=object(), scratch_specs=lambda: ()
        ),
    )
...
@pytest.mark.parametrize("draft_lane", [0, 1])
def test_dcp_auxiliary_warmup_covers_target_and_draft_without_repeating(
    monkeypatch, draft_lane
):
    from vllm.model_executor.warmup import dcp_prefill_warmup
    ...
    class Layer(torch.nn.Module):
        ...
        def prepare_dcp_prefill(self, counts):
            calls.append((self.role, _workspace_lane.get(), counts))
            return True

    worker = _worker(Layer("target"), draft=Layer("draft"), draft_lane=draft_lane)
    monkeypatch.setattr(dcp_prefill_warmup.current_platform, "is_cuda", lambda: True)
    monkeypatch.setattr(
        b12x_prepare, "b12x_workload", lambda *args, **kwargs: _workload("state")
    )
    monkeypatch.setattr(
        b12x_prepare,
        "_draft_workload",
        lambda worker, workload, *, lane: replace(workload, lane=lane),
    )
    assert dcp_prefill_warmup.warmup_dcp_prefill(worker, [8])
    assert calls == [
        ("target", 0, (1, 2, 4, 8, 32)),
        ("draft", draft_lane, (1, 2, 4, 8, 32)),
    ]
    assert not dcp_prefill_warmup.warmup_dcp_prefill(worker, [8])
    assert len(calls) == 2
    assert dcp_prefill_warmup.warmup_dcp_prefill(worker, [8, 16])
    assert len(calls) == 4 and all(16 in counts for _, _, counts in calls[2:])


def test_dcp_auxiliary_warmup_does_not_cache_a_failed_hook(monkeypatch):
    from vllm.model_executor.warmup import dcp_prefill_warmup
    ...
    class Layer(torch.nn.Module):
        def prepare_dcp_prefill(self, counts):
            calls.append(counts)
            if len(calls) == 1:
                raise RuntimeError("auxiliary compilation failed")
            return True
    ...
    with pytest.raises(RuntimeError, match="auxiliary compilation failed"):
        dcp_prefill_warmup.warmup_dcp_prefill(worker, [])
    assert dcp_prefill_warmup.warmup_dcp_prefill(worker, [])
    assert len(calls) == 2
