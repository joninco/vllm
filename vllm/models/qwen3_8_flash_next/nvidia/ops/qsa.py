...
def qsa_select_paged_tokens(
    ...
    for row_start in range(0, rows, rows_per_chunk):
        ...
        # The vLLM persistent/cooperative top-k operators return the right set
        # of indices but do not promise score order. QSA's public fixed-width
        # row follows torch.topk(sorted=True), with exact ties preferring the
        # lower group id. Canonicalize candidates by id first, then use a
        # stable score sort so expansion preserves that contract.
        visible = visible_blocks[:, None]
        valid_candidates = (blocks >= 0) & (blocks < visible) & (blocks < columns)
        candidates = torch.where(
            valid_candidates,
            blocks,
            blocks.new_full((), columns),
        )
        candidates = torch.sort(candidates, dim=1, stable=True).values
        candidate_scores = logits.gather(
            1, candidates.clamp_max(columns - 1).to(torch.long)
        )
        candidate_scores = torch.where(
            candidates < columns,
            candidate_scores,
            candidate_scores.new_full((), -torch.inf),
        )
        ...
        blocks.copy_(candidates.gather(1, score_order))
