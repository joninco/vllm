...
| `adaptive_speculative_tokens_window` | `integer > 0` | `None` | Adapt speculative depth from accepted draft lengths after this many non-empty verification steps. |
| `adaptive_speculative_tokens_initial` | `integer > 0` | `None` | Initial adaptive depth. Defaults to `num_speculative_tokens`, which remains the upper bound. |
